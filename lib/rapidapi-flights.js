...
function mapItinerary(raw, i, { origin, destination, departureDate }) {
  ...
  return {
    id: raw?.id || `${origin}-${destination}-${departureDate}-${i}`,
...
export async function searchFlights({
  ...
  if (!returnDate) {
    ...
    const json = await fetchItineraries({
      origin,
      destination,
      departureDate,
    });
  ...
  //
  // IMPORTANT:
  // Do NOT use the round-trip endpoint here.
  //
  // Instead perform two independent one-way searches:
  //
  ...
  if (usingMock()) {
    const outbound = buildMockOffers({
      origin,
      destination,
      departureDate,
    }).map((flight) => ({
  ...
  // Search OUTBOUND independently
  const outboundJson = await fetchItineraries({
    origin,
    destination,
    departureDate,
  });

  // Search RETURN independently
  const returnJson = await fetchItineraries({
    origin: destination,
    destination: origin,
    departureDate: returnDate,
  });

  const outboundRaw = outboundJson?.data?.itineraries || [];
  const returnRaw = returnJson?.data?.itineraries || [];
  ...
  const outbound = outboundRaw
    ...
    .map((flight) => ({
      ...
      // This is now an independent one-way price.
      isRoundTrip: true,
      tripLeg: 'outbound',

      // Make sure there is no return-leg information.
      returnDate: null,
  ...
  const inbound = returnRaw
    ...
    .map((flight) => ({
      ...
      // This is also an independent one-way price.
      isRoundTrip: true,
      tripLeg: 'return',
